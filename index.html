...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (data.stop_reason === 'max_tokens') {
      res.status(502).json({ error: 'Response got cut off before finishing (hit the token limit). Try again, or shorten the pasted question if it has many long answer choices.' });
      return;
    }
    ...
    try {
      ...
      res.status(502).json({
        error: 'Could not parse the analysis as JSON',
        detail: String(parseErr),
        tail: cleaned.slice(-300)
      });
      return;
    }
