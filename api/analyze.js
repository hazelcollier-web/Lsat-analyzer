...
- "formula_slots": derive this from the "formula" string itself. Every capitalized noun-phrase or [bracketed] placeholder in the formula text is a slot that needs a real-world filler from THIS question (e.g. in "Premise + Assumption → Conclusion", the slots are "Premise", "Assumption", "Conclusion"; in "Ask [Question] → Answer YES vs NO", the slot is "Question"). For each slot, output {"label": "the slot name exactly as it appears in the formula", "source": "stimulus"|"answer_choice", "ref": "the segment id (e.g. s2) if source is stimulus, or the letter (e.g. C) if source is answer_choice"}. Rules for filling: a slot naming something already stated in the passage (Premise, Conclusion, Fact A, Fact B, a speaker's claim) should reference a stimulus segment id. A slot that names something the correct answer choice provides (a new fact, an assumption to add, an explanation, a technique, a question to ask, an unjustified assumption [X], the matching parallel argument) should reference the letter of the correct answer choice — ONLY if answer choices were provided in the input; if no answer choices were given, omit any slot whose filler would have to come from them. Order slots the same order the terms first appear in the formula string.
- Respond with ONLY valid JSON, no markdown fences, no preamble. Schema:
{"question_type": "${VALID_TYPES.join('|')}", "segments": [{"id":"s1","text":"...","tag":"context","reason":"..."}], "identification_test": "...", "formula": "...", "formula_applied": "...", "formula_slots": [{"label":"Assumption","source":"stimulus","ref":"s2"}], "steps": ["...", "..."], "traps": ["...", "..."], "answer_choices": [{"letter":"A","text":"..."}], "answer_analysis": [{"letter":"A","verdict":"wrong","reason":"..."}]}`;
